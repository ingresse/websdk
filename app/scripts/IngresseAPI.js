...
function receiveMessage(event) {
  ...
  angular
    ...
    .scope()
    .$broadcast("ingresseAPI.userHasLogged", obj);
}
...
angular
  ...
  */
      ...
      API._getUrl = function (method, identifier, parameters) {
        var userToken =
          parameters && parameters.usertoken ? API._getRequestToken() : {};
        ...
        url += API._getUrlParameters(
          angular.extend({}, parameters || {}, userToken)
        );
      ...
      API._get = function (method, identifier, parameters, config) {
        ...
        $http.get(endpointUrl, config).then(
          function (response) {
            response = response.data;

            if (
              angular.isObject(response.responseData) &&
              angular.isDefined(response.ResponseSessionStatus)
            ) {
              response.responseData.sessionStatus =
                response.ResponseSessionStatus;
            }
      ...
      API._post = function (
        ...
        $http.post(endpointUrl, postParameters, config).then(
          function (response) {
            response = response.data;
      ...
      API._put = function (
        ...
        $http.put(endpointUrl, postParameters, config).then(
          function (response) {
            response = response.data;
      ...
      API._delete = function (method, identifier, parameters, config) {
        ...
        $http.delete(endpointUrl, config).then(
          function (response) {
            response = response.data;
      ...
      API.event = {
        ...
        getTicketTypesForSession: function (eventId, filters, usertoken) {
          var identifier =
            eventId + "/session/" + filters.sessionId + "/tickets";
      ...
      API.producer = {
        ...
        getSalesForCostumer: function (identifier, filters, token) {
          ...
          url =
            ...
            "/customer/" +
            identifier.costumerId +
            "/sale" +
            API._generateAuthKey();
      ...
      API.ticketBooth = {
        ...
        validate: function (postObject, token) {
          var filters = {
            usertoken: token,
          };
          return API._post("ticketbooth/validate", null, filters, postObject);
        },
      };
          ...
          API._get("user", userid, filters)
            ...
            .then(function (response) {
              ...
              if (filters.fields && filters.fields.indexOf("picture")) {
                userData.photo =
                  response.pictures && response.pictures.medium
                    ? response.pictures.medium
                    : response.picture;
              }
      ...
      API.login = {
        getAuthorizeUrl: function (returnUrl) {
          ...
          if (returnUrl) {
            url += "?returnUrl=" + API._urlencode(returnUrl);
          }
        ...
        getLogoutURL: function () {
          ...
          return (
            url +
            "&returnurl=" +
            API._urlencode(ingresseApiPreferences.loginReturnUrl)
          );
        },
      ...
      API.ticketReservation = function (
        eventId,
        userId,
        token,
        tickets,
      ...
      API.payReservation = function (
        ...
        if (paymentMethod === ingressePaymentType.CREDITCARD) {
          ...
          $http
            .post(url, payment.creditCardPayment(transaction))
            .catch(deferred.reject)
            .then(function (response) {
              response = response.data;
        ...
        // Generic payment
        $http
          .post(url, payment.genericPayment(transaction))
          .catch(deferred.reject)
          .then(function (response) {
            response = response.data;
      ...
      API.walletCardRemove = function (cardToken) {
        return API._delete("/wallet/creditcard", cardToken, {
          usertoken: true,
        });
      };
