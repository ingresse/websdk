...
.controller('EmulatorController', function ($rootScope, $scope, ipCookie, ingresseAPI, ingresseAPI_Preferences, IngresseAPI_UserService, IngresseAPI_Freepass, VenusActivityIndicatorService) {

  $rootScope.domain = ipCookie('host');

  if (!$rootScope.domain) {
    $rootScope.domain = ingresseAPI_Preferences._host;
  } else {
    ingresseAPI_Preferences.setHost($rootScope.domain);
  }
  ...
  $rootScope.setPrivateKey = function (key) {
    ingresseAPI_Preferences.setPrivateKey(key);
  };

  $rootScope.setPublicKey = function (key) {
  ...
  $scope.$watch('privateKey', function () {
    // $document.cookie = "privateKey=" + $rootScope.privateKey + "; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/";
    if (!$rootScope.privateKey) {
      return;
    }
    ipCookie('privatekey', $rootScope.privateKey, {expires: 365});
  });

  $scope.$watch('publicKey', function () {
    if (!$rootScope.publicKey) {
      return;
    }
    // $document.cookie = "publicKey=" + $rootScope.publicKey + "; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/";
    ipCookie('publickey', $rootScope.publicKey, {expires: 365});
  });
  ...
  $rootScope.privateKey = ingresseAPI_Preferences.privatekey;
  $rootScope.publicKey = ingresseAPI_Preferences.publickey;
});
