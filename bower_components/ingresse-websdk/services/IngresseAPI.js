...
angular.module('ingresseSDK').provider('ingresseAPI',function ($httpProvider) {
  ...
  $httpProvider.interceptors.push(function($q, ingresseAPI_Preferences, $log) {
    return {
      ...
      'responseError': function(rejection) {
        ...
        errorMessage = Error("Não foi possível nos comunicar com os servidores da ingresse. Verifique sua conexão com a internet e tente novamente.");
  ...
  return{
    $get: function($http, $rootScope, $q, ingresseAPI_Preferences) {
      return {
        ...
        */
        getEvent: function(eventId, fields, usertoken){
          ...
          if(angular.isNumber(parseInt(eventId)) && !isNaN(eventId)){
            var url = ingresseAPI_Preferences.getHost() + '/event/' + eventId + this.generateAuthKey();
          }else{
            var url = ingresseAPI_Preferences.getHost() + '/event/' + this.generateAuthKey() + '&method=identify&link=' + eventId;
          }
        ...
        getError: function (errorClass) {
          var deferred = $q.defer();
          if(errorClass){
            var url = ingresseAPI_Preferences.getHost() + '/error/' + errorClass + this.generateAuthKey();
          }else{
            var url = ingresseAPI_Preferences.getHost() + '/error/' + this.generateAuthKey();
          }
